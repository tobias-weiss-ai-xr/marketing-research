...
BURST_KEYWORDS = [
    "generative ai", "genai", "agentic", "llm", "large language", "chatbot",
    "influencer", "creator", "tiktok", "short-form", "reels", "ugc",
    "personaliz", "recommend", "retention", "loyalty", "churn",
    "attribution", "mmm", "marketing mix", "incrementality", "experiment",
    "cookieless", "first-party", "privacy", "consent",
    "retail media", "omnichannel", "customer journey", "cx",
    "brand", "storytelling", "purpose", "sustainability",
    "social commerce", "programmatic", "abm", "account-based", "b2b",
    "metaverse", "ar ", "virtual influencer", "voice",
]


def _bar(n, total, width=12):
    if not total:
        return ""
...
def update_readme_stats(readme_path, stats, bursts, top_venues, thin):
    """Regenerate the 'Corpus Statistics' section of README.md in place.

    Everything between the '## \U0001f4ca Corpus Statistics' and '## \U0001f4d6 Citation'
    markers is replaced; the curated intro/cross-refs are preserved.
    """
    start_marker = "## \U0001f4ca Corpus Statistics"
    end_marker = "## \U0001f4d6 Citation"
    ...
    total = m["total_papers"]
    src = stats["source_breakdown"]
    pct = lambda k: round(100 * src.get(k, 0) / total) if total else 0
    L = [start_marker, ""]
    L.append(f"**{total:,} papers** across **{m['taxonomy']['categories']} categories**.")
    L.append(f"Sources: **arXiv** {src.get('arxiv', 0)} ({pct('arxiv')}%) \u00b7 "
             f"**DOI** {src.get('doi', 0)} ({pct('doi')}%) \u00b7 "
             f"**Other** {src.get('other', 0)} ({pct('other')}%).")
    L.append("Full paper list: [GitHub Pages site](https://tobias-weiss-ai-xr.github.io/marketing-research).")
    L.append("")
    L.append("### Top categories")
    L.append("")
    L.append("| Category | Papers | Recent | |")
    L.append("|----------|--------|--------|-")
    for c, n in sorted(cats.items(), key=lambda kv: -kv[1]):
        mo = next((x for x in momentum if x["id"] == c), None)
        rec = mo["recent"] if mo else 0
        L.append(f"| {c} | **{n}** | {rec} | {_bar(n, max(cats.values()))} |")
    L.append("")
    L.append("### By year")
    L.append("")
    ...
    for y in sorted(years):
        L.append(f"| {y} | {years[y]} | {_bar(years[y], max(years.values()))} |")
    L.append("")
    L.append("### Momentum (hottest categories)")
    L.append("")
    L.append("| Category | Total | Rate | Recent | Score |")
    L.append("|----------|-------|------|--------|-------|")
    for mo in momentum[:5]:
        L.append(f"| {mo['id']} | {mo['total']} | {mo['papers_per_month']}/mo | "
                 f"{int(mo['recent_share'] * 100)}% | {mo['score']} |")
    L.append("")
    L.append("### Trending keywords")
    L.append("")
    L.append("| Keyword | Papers | Burst |")
    L.append("|---------|--------|-------|")
    for b in bursts[:8]:
        L.append(f"| {b['keyword']} | {b['total']} | {b['burst_score']} |")
    L.append("")
    L.append("### Top venues")
    L.append("")
    L.append("| Venue | Papers |")
    L.append("|-------|--------|")
    for v in top_venues[:8]:
        L.append(f"| {v['name']} | {v['papers']} |")
    L.append("")
    L.append("### Research gaps (thinnest cells)")
    L.append("")
    L.append("| Cell | Papers |")
    L.append("|------|--------|")
    for c, n in thin[:8]:
        L.append(f"| `{c}` | {n} |")
    L.append("")
    L.append("*Generated by `scripts/standard_stats.py`.*")
    L.append("")
    readme_path.write_text(before + "\n".join(L) + "\n" + after, encoding="utf-8")
    print("Updated README Corpus Statistics section")
...
def main():
    ...
    update_readme_stats(REPO / "README.md", stats, bursts, top_venues, thin)
