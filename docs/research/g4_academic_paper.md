...
Total profit by environment (mean over 50 seeds; full data incl. paired F3 statistics: `results/cam_sim_results_robustness.md`). Across all nine environments, the paired seed-level `situation_only − oracle` difference is significant (8× pro-F3, p ≤ 3.5e-44; 1× reversal under concave_returns, p = 2.2e-07):
...
**F7 (label-free dose-response — dominant, not exceptionless):** Label-ordered ladders can mislead (a learned classifier at 87.5% *should* exceed an 80%-perception agent). The label-free test — Spearman ρ(context match rate, profit) across agents within each environment — yields **ρ ≥ 0.93 in all nine environments**. We report ρ as a *descriptive monotonicity check*: the nine agents share one generator and reward table and are not independent draws, so no significance is attached. And F6's uniform exception shows the moderator: systematic errors into payoff-asymmetric actions can suppress profit below what match rate alone predicts.

**F8 (boundary conditions of the bid surprise):** F3 holds with paired significance in 8/9 environments (all p ≤ 3.5e-44; see table note) — including under budget constraints, where the oracle's over-bidding burns budget (+$166.31 → +$153.98) while the baseline *improves* as truncation stops its bleeding (−$180.31 → −$134.22; per-agent forfeited moments range 0.0 [situation_only, never truncates] to 37.5/200 [baseline]; agents are budget-*unaware* — the constraint tests spending efficiency, not planning). The single reversal is `concave_returns` and is itself significant (paired diff −$6.11, CI [−8.10, −4.12], p = 2.2e-07, d = −0.41). Mechanism caveat: under concave returns the reward multiplier caps at 2×, and the flat bid of 1.0 far exceeds the low-intent clearing price (~0.15–0.2), so `situation_only` *harvests the capped multiplier* on low-intent contexts — the reversal compares two suboptimal heuristics under one curvature (α = 0.5), not calibrated bidding against an optimum. **Bid modulation gains value exactly when — and, at this α, only when — the environment prices reach.**
...
- **H4 (Dose-response):** ✅ Supported — label-free ρ(match rate, profit) ≥ 0.93 in all 9 environments (F7, descriptive); label-ordering holds under default economics and breaks only for the biased hand-tuned classifier under distribution shift (F5), which recalibration largely fixes (F6)
...
4. **Between-environment robustness: tested.** The full ladder replicates across **9 environment presets** spanning distribution shifts, doubled costs, budget caps, concave returns to bid, and weakened context payoffs (§8.4): the label-free dose-response is dominant (ρ ≥ 0.93, descriptive); F3 holds with paired significance in 8/9 and reverses significantly when the environment prices reach (F8). Remaining scope: adversarial contexts, multi-period state carryover, auction-style clearing, and additional curvatures (single α = 0.5 tested).
